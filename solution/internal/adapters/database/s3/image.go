...
func (s *imageStorage) UploadImage(ctx context.Context, image *multipart.File, contentType string, campaignId uuid.UUID, size int64) error {
...
func (s *imageStorage) GetImage(ctx context.Context, campaignId uuid.UUID) ([]byte, error) {
	...
	_, err := s.client.StatObject(ctx, s.bucket, fmt.Sprintf("%s", campaignId.String()), minio.StatObjectOptions{})
	if err != nil {
		return nil, errorz.NotFound
	}

	object, err := s.client.GetObject(ctx, s.bucket, fmt.Sprintf("%s", campaignId.String()), minio.GetObjectOptions{})
...
func (s *imageStorage) DeleteImage(ctx context.Context, campaignId uuid.UUID) error {
	...
	return s.client.RemoveObject(ctx, s.bucket, fmt.Sprintf("%s", campaignId), minio.RemoveObjectOptions{})
}
